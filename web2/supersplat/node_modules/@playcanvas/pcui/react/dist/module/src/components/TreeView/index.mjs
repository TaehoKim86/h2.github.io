 ...
 * {@link TreeViewItem}s.
 */
let TreeView$1 = class TreeView extends Container$1 {
    /**
    ...
    constructor(args = {}) {
        ...
        this._dragHandle = new Element$1({
    ...
    _findNextVisibleTreeItem(currentItem) {
        ...
        if (!(parent instanceof TreeViewItem$1))
            return null;
    ...
    _onAppendTreeViewItem(item) {
        ...
        item.forEachChild((child) => {
            if (child instanceof TreeViewItem$1) {
            ...
            case 'ArrowRight': {
                if (item.numChildren > 0) {
                    if (!item.open) {
                        ...
                        if (firstChild instanceof TreeViewItem$1) {
    ...
    _traverseDepthFirst(fn) {
        function traverse(item) {
            if (!item || !(item instanceof TreeViewItem$1))
                return;
            fn(item);
            if (item.numChildren) {
    ...
    clearTreeItems() {
        ...
        while (i--) {
            ...
            if (!dom)
                continue;
            const ui = dom.ui;
            if (ui instanceof TreeViewItem$1) {
                ui.destroy();
            }
        }
        this._selectedItems = [];
